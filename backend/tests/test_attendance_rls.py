"""RLS owner-isolation enforcement test for ``attendance_records`` (P3 Task 14).

``attendance_records`` is a P3 student-owned table (Decision 2). Its migration
(``b2e4d8f1a067``) enables ROW LEVEL SECURITY and an owner-isolation policy keyed
...
_INSERT_MEETING = text(
    "INSERT INTO course_meetings (id, course_id, meeting_index, scheduled_at, duration_minutes, status) "
    "VALUES (CAST(:id AS uuid), CAST(:c AS uuid), 1, now(), 60, 'planned')"
)
_INSERT_ATTENDANCE = text(
    "INSERT INTO attendance_records "
    "(id, meeting_id, user_id, status, source) "
    "VALUES (CAST(:id AS uuid), CAST(:m AS uuid), CAST(:u AS uuid), 'present', 'qr')"
)
_SET_GUC = text("SELECT set_config('app.current_user_id', :u, false)")
_COUNT = text("SELECT count(*) FROM attendance_records")
...
@pytest.mark.asyncio
async def test_attendance_rls_owner_isolation(async_engine):
    """User A's attendance row is invisible + immutable to user B under RLS.
    ...
    async with async_engine.connect() as conn:
        ...
        try:
            ...
            await conn.execute(_INSERT_MEETING.bindparams(id=meeting_id, c=course_id))
            await conn.commit()
            ...
            await conn.execute(_INSERT_ATTENDANCE.bindparams(id=row_a, m=meeting_id, u=user_a))
            ...
            upd = await conn.execute(
                text("UPDATE attendance_records SET status = 'absent' WHERE user_id = CAST(:u AS uuid)").bindparams(u=user_a)
            )
            assert upd.rowcount == 0, "B must not be able to UPDATE A's row"

            dele = await conn.execute(
                text("DELETE FROM attendance_records WHERE user_id = CAST(:u AS uuid)").bindparams(u=user_a)
            )
            ...
            with pytest.raises(DBAPIError) as check_exc:
                await conn.execute(
                    _INSERT_ATTENDANCE.bindparams(id=str(uuid.uuid4()), m=meeting_id, u=user_a)
                )
            ...
            await conn.execute(
                text("DELETE FROM course_meetings WHERE id = CAST(:m AS uuid)").bindparams(m=meeting_id)
            )
            await conn.execute(
