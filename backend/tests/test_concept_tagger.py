...
@pytest.mark.asyncio
async def test_tag_chunk_via_llm_drops_tiny_weights(db_session, monkeypatch):
    """A weight that quantizes to 0.00 must not produce an inserted row."""
    from app.models import Chunk, Concept, ConceptTag, Course, Document, User

    user = User(better_auth_id="i2", email="i2@ust.hk", role="instructor", full_name="i2")
    db_session.add(user)
    await db_session.commit()
    course = Course(instructor_id=user.id, name="C2", language="english", enroll_code="X0002")
    db_session.add(course)
    await db_session.commit()
    doc = Document(
        course_id=course.id, filename="x.pdf", file_type="pdf",
    ...
    await db_session.commit()
    concept = Concept(
        course_id=course.id, name="Edge case", status="approved", instructor_curated=True,
    )
    db_session.add(concept)
    await db_session.commit()

    async def fake_llm(text, candidates):
        return [{"concept_id": str(concept.id), "weight": 0.001}]

    monkeypatch.setattr(concept_tagger, "_llm_tag_call", fake_llm)

    inserted = await tag_chunk_via_llm(
        db_session,
        chunk_id=chunk.id,
        chunk_text="anything",
        course_id=course.id,
