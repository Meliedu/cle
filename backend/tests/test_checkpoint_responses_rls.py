"""RLS owner-isolation enforcement test for ``checkpoint_responses`` (P3 Task 14).

``checkpoint_responses`` is a P3 student-owned table (Decision 2). Its migration
(``a1f3c7e29b04``) enables ROW LEVEL SECURITY and an owner-isolation policy keyed
on the ``app.current_user_id`` GUC:
...
mirroring the ``tests/test_readiness_rls.py`` precedent. It runs against
...
_INSERT_CHECKPOINT = text(
    "INSERT INTO checkpoints (id, course_id, kind, status, title) "
    "VALUES (CAST(:id AS uuid), CAST(:c AS uuid), 'session', 'published', 'RLS Probe')"
)
_INSERT_CARD = text(
    "INSERT INTO checkpoint_cards (id, checkpoint_id, position, kind, prompt) "
    "VALUES (CAST(:id AS uuid), CAST(:cp AS uuid), 0, 'review_point', 'probe?')"
)
_INSERT_RESPONSE = text(
    "INSERT INTO checkpoint_responses "
    "(id, checkpoint_id, card_id, user_id, confidence, status) "
    "VALUES (CAST(:id AS uuid), CAST(:cp AS uuid), CAST(:card AS uuid), "
    "CAST(:u AS uuid), 1, 'on_time')"
)
_SET_GUC = text("SELECT set_config('app.current_user_id', :u, false)")
_COUNT = text("SELECT count(*) FROM checkpoint_responses")
...
@pytest.mark.asyncio
async def test_checkpoint_responses_rls_owner_isolation(async_engine):
    """User A's checkpoint response is invisible + immutable to user B under RLS.
    ...
    async with async_engine.connect() as conn:
        ...
        try:
            ...
            await conn.execute(_INSERT_CHECKPOINT.bindparams(id=checkpoint_id, c=course_id))
            await conn.execute(_INSERT_CARD.bindparams(id=card_id, cp=checkpoint_id))
            await conn.commit()
            ...
            await conn.execute(
                _INSERT_RESPONSE.bindparams(id=row_a, cp=checkpoint_id, card=card_id, u=user_a)
            )
            ...
            upd = await conn.execute(
                text("UPDATE checkpoint_responses SET status = 'late' WHERE user_id = CAST(:u AS uuid)").bindparams(u=user_a)
            )
            assert upd.rowcount == 0, "B must not be able to UPDATE A's row"

            dele = await conn.execute(
                text("DELETE FROM checkpoint_responses WHERE user_id = CAST(:u AS uuid)").bindparams(u=user_a)
            )
            ...
            with pytest.raises(DBAPIError) as check_exc:
                await conn.execute(
                    _INSERT_RESPONSE.bindparams(
                        id=str(uuid.uuid4()), cp=checkpoint_id, card=card_id, u=user_a
                    )
                )
            ...
            await conn.execute(
                text(
                    "DELETE FROM checkpoint_responses "
                    "WHERE user_id IN (CAST(:a AS uuid), CAST(:b AS uuid))"
                ).bindparams(a=user_a, b=user_b)
            )
            await conn.execute(
                text("DELETE FROM checkpoint_cards WHERE id = CAST(:card AS uuid)").bindparams(card=card_id)
            )
            await conn.execute(
                text("DELETE FROM checkpoints WHERE id = CAST(:cp AS uuid)").bindparams(cp=checkpoint_id)
            )
            await conn.execute(
