"""RLS owner-isolation enforcement test for ``work_item_progress`` (P4 Task B10).

``work_item_progress`` is a P4 student-owned table (Decision 2). Its migration
(``e6c2b8f4a19d``) enables ROW LEVEL SECURITY and an owner-isolation policy keyed
on the ``app.current_user_id`` GUC (copied verbatim from ``d94257fc717c`` /
the ``checkpoint_responses`` precedent):
...
mirroring the ``tests/test_checkpoint_responses_rls.py`` precedent. It runs
against ``async_engine`` — the migrated dev DB where the policy actually exists —
because the ``db_session`` fixture builds schema via ``Base.metadata.create_all``,
which never emits RLS/policy statements.
...
The operational spine (``work_items``) is deliberately NO-RLS (endpoint-guarded)
— here it is only seeded as the FK parent of a progress row.
"""
import uuid
...
_INSERT_WORK_ITEM = text(
    "INSERT INTO work_items "
    "(id, course_id, source_kind, source_id, title, required, score_bearing, created_by) "
    "VALUES (CAST(:id AS uuid), CAST(:c AS uuid), 'material', CAST(:src AS uuid), "
    "'RLS Probe Item', true, false, CAST(:by AS uuid))"
)
_INSERT_PROGRESS = text(
    "INSERT INTO work_item_progress "
    "(id, work_item_id, user_id, status) "
    "VALUES (CAST(:id AS uuid), CAST(:wi AS uuid), CAST(:u AS uuid), 'pending')"
)
_SET_GUC = text("SELECT set_config('app.current_user_id', :u, false)")
_COUNT = text("SELECT count(*) FROM work_item_progress")
...
@pytest.mark.asyncio
async def test_work_item_progress_rls_owner_isolation(async_engine):
    """User A's progress row is invisible + immutable to user B under RLS.
    ...
    async with async_engine.connect() as conn:
        ...
        try:
            ...
            await conn.execute(
                _INSERT_WORK_ITEM.bindparams(
                    id=work_item_id, c=course_id, src=source_id, by=instructor
                )
            )
            await conn.commit()
            ...
            await conn.execute(
                _INSERT_PROGRESS.bindparams(id=row_a, wi=work_item_id, u=user_a)
            )
            ...
            upd = await conn.execute(
                text("UPDATE work_item_progress SET status = 'completed' WHERE user_id = CAST(:u AS uuid)").bindparams(u=user_a)
            )
            assert upd.rowcount == 0, "B must not be able to UPDATE A's row"

            dele = await conn.execute(
                text("DELETE FROM work_item_progress WHERE user_id = CAST(:u AS uuid)").bindparams(u=user_a)
            )
            ...
            with pytest.raises(DBAPIError) as check_exc:
                await conn.execute(
                    _INSERT_PROGRESS.bindparams(
                        id=str(uuid.uuid4()), wi=work_item_id, u=user_a
                    )
                )
            ...
            await conn.execute(
                text(
                    "DELETE FROM work_item_progress "
                    "WHERE user_id IN (CAST(:a AS uuid), CAST(:b AS uuid))"
                ).bindparams(a=user_a, b=user_b)
            )
            await conn.execute(
                text("DELETE FROM work_items WHERE id = CAST(:wi AS uuid)").bindparams(wi=work_item_id)
            )
            await conn.execute(
