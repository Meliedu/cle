...
async def _verify_instructor_enrollment(
    ...
    # Delegate to the shared helper, which requires an ACTIVE, instructor-role
    # enrollment in a non-deleted course. The previous local check accepted ANY
    # enrollment row (any role, any status), so a `require_instructor` user who
    # held a pending/rejected — or student-role — enrollment in a course could
    # read that course's full student analytics (names, emails, scores).
    await verify_instructor_enrollment(db, course_id, user_id)
